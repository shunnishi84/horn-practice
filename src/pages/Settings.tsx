...
export default function Settings() {
  ...
  return (
    <div className="space-y-4" data-testid="settings-page">
      ...
      <Field label={`楽器`}>
        <select className="bg-slate-800 px-2 py-1 rounded" value={settings.instrument} onChange={(e) => updateSettings({ instrument: e.target.value as any })}>
          {INSTRUMENT_LIST.map((i) => <option key={i.key} value={i.key}>{i.displayName}</option>)}
        </select>
      </Field>
      <Field label="移調">
        <select className="bg-slate-800 px-2 py-1 rounded" value={settings.transposition} onChange={(e) => updateSettings({ transposition: e.target.value as any })}>
      ...
      <Field label={`チューニング基準: ${settings.tuningHz} Hz`}>
      ...
      <Field label="マイクデバイス">
        <select className="bg-slate-800 px-2 py-1 rounded" value={settings.inputDeviceId ?? ''} onChange={(e) => updateSettings({ inputDeviceId: e.target.value || undefined })}>
          <option value="">自動選択</option>
          {devices.map((d) => <option key={d.deviceId} value={d.deviceId}>{d.label || d.deviceId.slice(0, 6)}</option>)}
        </select>
      </Field>
      <Field label="テーマ">
        <select className="bg-slate-800 px-2 py-1 rounded" value={settings.theme} onChange={(e) => updateSettings({ theme: e.target.value as any })} data-testid="theme-select">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="bg-slate-800 rounded p-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
