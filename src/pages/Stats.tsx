    ...
    <div className="space-y-8" data-testid="stats-page">
      <section>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h1 className="text-2xl font-extrabold">🗓️ 活動カレンダー</h1>
          <div className="flex gap-2 text-sm">
            <button
              onClick={() => setCalMetric('time')}
              className={calMetric === 'time' ? 'chip-on' : 'chip'}
            >練習時間</button>
            <button
              onClick={() => setCalMetric('score')}
              className={calMetric === 'score' ? 'chip-on' : 'chip'}
            >平均スコア</button>
          </div>
        </div>
        <div className="text-sm font-bold text-muted mt-2">🔥 現在のストリーク <span className="text-pop-pink">{streak.current} 日</span> / 最長 {streak.longest} 日</div>
        <div className="card p-4 mt-3"><HeatmapCalendar daily={daily} metric={calMetric} days={365} /></div>
      </section>

      <section>
        <h2 className="text-lg font-extrabold mb-3">📈 週ごとの練習時間</h2>
        <div className="card p-4">
          <div className="flex items-end gap-1.5 h-32" data-testid="weekly-bars">
            {weekly.map((w, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                <div
                  className="w-full bg-gradient-to-t from-pop-violet to-pop-pink rounded-t-md"
                  style={{ height: `${Math.min(100, w.mins * 4)}%` }}
                  title={`${w.mins.toFixed(1)} 分`}
                />
                <div className="text-[10px] font-bold text-muted">{w.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h2 className="text-lg font-extrabold">🔍 苦手音分析</h2>
          <div className="flex gap-2 text-sm flex-wrap">
            <select className="select-pop" value={periodDays ?? 'all'} onChange={(e) => setPeriodDays(e.target.value === 'all' ? undefined : parseInt(e.target.value))}>
              <option value="all">全期間</option>
              <option value="7">直近7日</option>
              <option value="30">直近30日</option>
            </select>
            <select className="select-pop" value={octaveFilter} onChange={(e) => setOctaveFilter(e.target.value as any)}>
              <option value="all">全音域</option>
              <option value="low">低音域</option>
              <option value="mid">中音域</option>
              <option value="high">高音域</option>
            </select>
            <select className="select-pop" value={dirFilter} onChange={(e) => setDirFilter(e.target.value as any)}>
              ...
              {stats
                ...
                .map((s, i) => (
                  <tr key={i} className="border-t-2 border-line">
                    <td className="px-2 py-1 font-bold text-pop-violet">{s.pitchClass}</td>
